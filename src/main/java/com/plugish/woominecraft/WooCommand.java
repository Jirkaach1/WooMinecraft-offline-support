...
public class WooCommand implements TabExecutor {
    ...
    private void clearLogSubcommand(CommandSender sender) {
        if (!sender.hasPermission("woo.admin")) {
        ...
        try {
            ...
            sender.sendMessage(chatPrefix + ChatColor.RED + "An error occurred while clearing the log file: " + e.getMessage());
            e.printStackTrace();
        }
    }
